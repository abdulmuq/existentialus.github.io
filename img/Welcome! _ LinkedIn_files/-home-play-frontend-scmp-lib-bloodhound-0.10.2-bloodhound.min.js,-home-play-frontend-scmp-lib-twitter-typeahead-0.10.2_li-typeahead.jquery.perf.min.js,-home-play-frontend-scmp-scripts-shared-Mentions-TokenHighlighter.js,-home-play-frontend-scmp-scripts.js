...
b=a.indexOf("MSIE ");return 0<b||navigator.userAgent.match(/Trident.*rv\:11\./)?parseInt(a.substring(b+5,a.indexOf(".",b)),10):0}(),containerClass:".mentions-container",idAttr:"data-li-mentionsid",discussionIdAttr:"data-li-discussionid",hasDelegatorSelector:".hasMentionsDelegator"});b.prototype={setInstance:function(a){a=g(a.target).closest(b.containerClass);var c=a.attr(b.discussionIdAttr)||0,e=this.config.isInfluencer,d=this.config.isMegaphone||!1,f=this.config.oonMentions,k=this.config.schoolMentions,
l=this.config.limits,m=this.config.mentionsTL;a.attr(b.idAttr)||(c=new h(a.get(0),{isInfluencer:null!==e?e:b.isInfluencer,ieVersion:b.ieVersion,discussionId:c,memberId:this.memberId,oonMentions:f,isMegaphone:d,schoolMentions:k,limits:l,mentionsTL:m}),a.attr(b.idAttr,b.count),b.cache[b.count]={el:a,instance:c},b.count++)},getInstance:function(a){a=g(a).closest(b.containerClass).attr(b.idAttr);if(void 0===a)throw Error("Undefined -- no Mentions instance is associated with that element. Make sure to pass in the .mentions-container element");
return b.cache[a].instance},checkInfluencer:function(){var a;void 0===b.isInfluencer&&(a=d.bind(function(a){try{this.isInfluencer=a.content.influencer_entitlement.result.isInfluencer}catch(b){}},b),g.ajax({url:"/nhome/influencer-entitlement",beforeSend:function(a){a.setRequestHeader("X-IsAJAXForm","1")},success:a}))},init:function(){var a=g(this.el),c=b.hasDelegatorSelector;if(a.is(c))throw Error("Mentions Delegator cannot be instantiated twice on the same element");if(a.find(c).length)throw Error("Mentions Delegator cannot be nested (a Delegator already exists within that DOM scope)");
if(a.parents(c).length)throw Error("Mentions Delegator cannot be nested (tried to instantiate a Delegator within a Delegator");this.checkInfluencer();d.bindAll(this,"setInstance");a.addClass(c.substr(1));a.on("focus",".mentions-input",this.setInstance)}};h.Delegator=b})(window.LIModules);(function(d){var c=d.requires("underscore"),k=d.requires("jquery"),e=d.requires("BaseControl"),g=d.requires("asyncRequest"),h=d.requires("Events");d.exports("scripts.FollowToggler",e.extend(function(d){var e={urlAttribute:"href",toggleClass:"is-following",toggleContainerClass:"",actionClassSuffix:"",globalEventSuffix:"",selectedClass:"",loadingClass:"loading",requestMethod:"GET"};return{init:function(a,b){c.defaults(b,e);d.init.call(this,a,b)},beforeDecoration:function(){var a=this._config;this._globalEventSuffix=
